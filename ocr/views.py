...
def download_pdf(request):
    ...
    if request.POST.get("pdf"):
        ...
        # This file name is the default file name when downloading and can be changed
...
def homepage(request):
    ...
    if request.method == 'POST' and UserFileForm(request.POST, request.FILES).is_valid():
        ...
        file_data = request.POST["txtareaField"]
        textlines = file_data.split('.')
        Language = request.POST.get("lang", None)
        translator = Translator(to_lang=Language)
        translate_text = ""
        for line in textlines:
            translate_text = translate_text + translator.translate(line)
...
def userLogin(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.info(
                request, 'Username or password is invalid, Please check it once.!')
...
def userSignup(request):
    if request.method == "POST":
        ...
        try:
            ...
            messages.info(
                request, 'Your username is not unique. try another one')
            return redirect('homepage')
            ...
            messages.info(
                request, 'Welcome to Django-ocr app! You have created your account successfully')
            return redirect('homepage')
    else:
        return render(request, 'ocr/error404.html')
